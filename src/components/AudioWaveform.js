...
import { RecordingContext } from './AudioRecorder'
...
const AudioWaveform = () => {
	...
	useEffect(() => {
		if (recording && wavesurferObj) {
			wavesurferObj.load(recording);
		}
	}, [recording, wavesurferObj]);
...
console.log("1",first_list_index);
console.log("2", second_list_index);
console.log("memory", second_list_mem_alloc);
...
console.log(original_buffer.copyFromChannel(new_list, 0));				
...
console.log(new_buffer.copyToChannel(combined, 0));			
